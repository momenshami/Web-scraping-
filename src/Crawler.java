    ...
    public ArrayList<String> getNewUsingBoilerPipe(ArrayList<String> linksList) throws IOException {
        ...
        try {
            for (int i = 0; i < linksList.size(); i++) {
                ...
                URL url = new URL(linksList.get(i).toString());
                ...
                try {
                    if (HTMLFetcher.fetch(new URL(linksList.get(i))) == null)
                        System.out.println("ther is no text in the link");
                } catch (IOException e) {
                    content = ifNull;
                    System.out.println("***********************************************************\n\n\n\n\n\n");
        ...
        for (int i = 0; i < newlinks.size(); i++) {
            if (newlinks.size() == newsList.size()) {
                ...
                if (collection.find().first() == null) {
                   ...
                   // System.out.println("we add new links" + newlinks.get(i));
                    ...
                    if (collection.find(filter).first() == null) {
                        collection.insertOne(toInsert);
                       // System.out.println("we add new links");
                    } else {
